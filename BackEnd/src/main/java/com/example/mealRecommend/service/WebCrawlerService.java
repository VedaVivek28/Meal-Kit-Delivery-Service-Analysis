...
import com.example.mealRecommend.util.WebDriverUtil;
...
public class WebCrawlerService {
    ...
    private static List<Recipe> mergedRecipes = new ArrayList<>();
        ...
        public synchronized void init() {
            ...
            allRecipes.addAll(runScraper("FreshPrep", this::scrapeRecipesFromFreshPrep));
            // allRecipes.addAll(runScraper("HelloFresh", () -> webScraper.scrapeRecipes())); // Commented out: HelloFresh blocks scraping (HTTP 403).
            allRecipes.addAll(runScraper("HomeChef", VedaService::scrapeRecipes));
            allRecipes.addAll(runScraper("Dinnerly", DinnerlyRecipeScraper::scrapeRecipes));
            allRecipes.addAll(runScraper("SnapKitchen", SnapKitchenService::scrapeRecipes));
        ...
        private List<Recipe> runScraper(String name, Supplier<List<Recipe>> scraper) {
            try {
                System.out.println("Running " + name + " scraper...");
                List<Recipe> recipes = scraper.get();
                System.out.println(name + " scraper returned " + recipes.size() + " recipes");
                return recipes;
            } catch (Exception e) {
                System.err.println(name + " scraper failed: " + e.getMessage());
                e.printStackTrace();
