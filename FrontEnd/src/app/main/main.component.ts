...
export class MainComponent implements OnInit {
  ...
  handleSearch(term: string) {
    ...
    this.http.post<SearchResponse>(`http://localhost:8080/search/recipes?query=${term}`, {}).subscribe(
      (response) => {
        this.filteredServices = response.recipes;
        this.searchWordCount = response.wordCount;
        this.scrapedDataFrequencies = response.wordFrequencies;
  ...
  getSuggestedWord(term: string) {
    this.http.get<string[]>(`http://localhost:8080/search/suggestedwords?prefix=${term}`).subscribe(
      (data) => {
  ...
  onSearchInput(): void {
    ...
    if (searchTerm.length > 2) {
      this.getWordCompletions(searchTerm);
    } else {
